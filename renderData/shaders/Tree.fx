	...
	uniform float4x4 ViewInverse, uniform float4x4 WorldViewProjection, uniform float3 LightDir)
{
	...
	//WIND
	// float x = sin(IN.position.x / swayRigidness + (time * windSpeed)) * (IN.position.y - yOffset);
    // float z = sin(IN.position.z / swayRigidness + (time * windSpeed)) * (IN.position.y - yOffset);

	// IN.position.x += (step(0, IN.position.y - yOffset) * x * swayMax);
    // IN.position.z += (step(0, IN.position.y - yOffset) * z * swayMax);

    float x = sin(IN.position.x / swayRigidness + (time * windSpeed));
    float y = sin(IN.position.y / swayRigidness + (time * windSpeed));
    float z = sin(IN.position.z / swayRigidness + (time * windSpeed));

	IN.position.x += (step(0, IN.position.y - yOffset) * x * swayMax);
    IN.position.y += (step(0, IN.position.y - yOffset) * y * swayMax);
    IN.position.z += (step(0, IN.position.y - yOffset) * z * swayMax);
    ...
    // float x = sin(worldPos.x / swayRigidness + (time * windSpeed)) * (worldPos.y - yOffset);
    // float z = sin(worldPos.z / swayRigidness + (time * windSpeed)) * (worldPos.y - yOffset);

	// worldPos.x += (step(0, worldPos.y - yOffset) * x * swayMax);
    // worldPos.z += (step(0, worldPos.y - yOffset) * z * swayMax);
...
float4 frag(vOutput IN) : COLOR
{
    ...
    if (light0Enable)
    {
        ...
        float3 totalSpec = specular + fresnel;

        // SHADOW
		float shadow = 1.0f;
		shadow = ComputeShadows(IN);

        float3 totalLight = (diffuse + totalSpec) * light0Color * light0Intensity;
